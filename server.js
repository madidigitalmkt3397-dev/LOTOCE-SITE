...
// ---------------------------------------------------------------------
// ANÁLISE — resumo real (sem dados inventados; se não houver dados
// suficientes, o próprio endpoint informa isso, conforme regra 68/38 do prompt)
// ---------------------------------------------------------------------
app.get('/api/analysis/summary', async (req, res) => {
  try {
    const totais = await pool.query(`
      SELECT
        (SELECT COUNT(*) FROM draws) AS total_sorteios,
        (SELECT COUNT(*) FROM draw_results) AS total_resultados,
        (SELECT MIN(draw_date) FROM draws) AS data_mais_antiga,
        (SELECT MAX(draw_date) FROM draws) AS data_mais_recente
    `);

    const porSessao = await pool.query(`
      SELECT sc.code, COUNT(*) AS total
      FROM draws d JOIN sessions_config sc ON sc.id = d.session_id
      GROUP BY sc.code ORDER BY sc.code
    `);

    // Frequência real por milhar (universo 0000-9999) — só faz sentido mostrar
    // um recorte (top mais/menos frequentes) já que são 10.000 possibilidades.
    ...
    const totalResultados = parseInt(totais.rows[0].total_resultados, 10);
    const MINIMO_PARA_ANALISE = 50; // limiar simples e conservador — nada de fingir robustez estatística com poucos dados

    res.json({
      totalSorteios: parseInt(totais.rows[0].total_sorteios, 10),
      totalResultados,
      dataMaisAntiga: totais.rows[0].data_mais_antiga,
      dataMaisRecente: totais.rows[0].data_mais_recente,
      porSessao: porSessao.rows,
      dadosSuficientesParaAnalise: totalResultados >= MINIMO_PARA_ANALISE,
      minimoRecomendado: MINIMO_PARA_ANALISE,
      top10MaisFrequentes: totalResultados >= MINIMO_PARA_ANALISE ? frequencia.rows : [],
      aviso: totalResultados < MINIMO_PARA_ANALISE
        ? `Dados insuficientes para análise de frequência confiável (há ${totalResultados} resultado(s) gravado(s); recomendo pelo menos ${MINIMO_PARA_ANALISE}).`
        : null
    });
  } catch (e) {
    res.status(500).json({ erro: 'Falha ao calcular o resumo.', detalhe: e.message });
  }
});
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.status(500).json({ status: 'ok', database: 'disconnected', detalhe: e.message });
  }
});
